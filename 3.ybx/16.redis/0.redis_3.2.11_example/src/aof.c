...
}//end of void aofRewriteBufferAppend()
...
}//end of void stopAppendOnly(void)
...
int startAppendOnly(void) {
    ...
    return C_OK;
}//end of int startAppendOnly(void)
...
// 将AOF缓存写到磁盘中
// 因为我们需要在回复client之前对AOF执行写操作，唯一的机会是在事件loop中，
// 因此累计所有的AOF到缓存中，在下一次重新进入事件loop之前将缓存写到AOF文件中

// 关于force参数
// 当fsync被设置为每秒执行一次，如果后台仍有线程正在执行fsync操作，我们可能会延迟flush操作，
// 因为write操作可能会被阻塞，当发生这种情况时，说明需要尽快的执行flush操作，
// 会调用 serverCron() 函数。
...
void flushAppendOnlyFile(int force) {
    ...
    if (server.aof_fsync == AOF_FSYNC_EVERYSEC && !force) {
        ...
        // 如果同步已经在后台执行，那么可以延迟两秒，如果设置了force，
        // 那么服务器会阻塞在write操作上
    ...
    // 如果no-appendfsync-on-rewrite被设置为yes，表示正在执行重写的时候，则不执行fsync
...
}//end of void flushAppendOnlyFile()
...
}//end of catAppendOnlyGenericCommand()
...
sds catAppendOnlyExpireAtCommand(sds buf, struct redisCommand *cmd, robj *key, robj *seconds) {
    ...
    // 返回buf
    return buf;
}//end of sds catAppendOnlyExpireAtCommand()
...
void feedAppendOnlyFile(struct redisCommand *cmd, int dictid, robj **argv, int argc) {
   ...
   // 如果正在进行AOF，则将命令追加到AOF的缓存中，在重新进入事件循环之前，这些命令会被冲洗到磁盘上，并向client回复
    ...
    sdsfree(buf);
}//end of void feedAppendOnlyFile()
...
fmterr: /* Format error. */
    ...
    exit(1);
}//end of int loadAppendOnlyFile(char *filename)
...
int rewriteSortedSetObject(rio *r, robj *key, robj *o) {
    ...
    return 1;
}//end of int rewriteSortedSetObject()
...
werr:
    ...
    return C_ERR;
}//end of int rewriteAppendOnlyFile(char *filename)
...
// 在重写期间创建一个用来在父子进程通信的管道。有一个数据管道用于子进程发送AOF增量差异，
// 另外两个由子进程使用完成重写操作，所以不应该写入更多的数据，
// 另一个用于父进程发送ack应答信号取确认这个新条件。
...
error:
    ...
    return C_ERR;
}//end of int aofCreatePipes(void)
...
}//end of int rewriteAppendOnlyFileBackground(void)
...
cleanup:
    ...
    if (server.aof_state == AOF_WAIT_REWRITE)
        server.aof_rewrite_scheduled = 1;
}//end of void backgroundRewriteDoneHandler()
