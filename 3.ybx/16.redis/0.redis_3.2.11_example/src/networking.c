...
void freeClient(client *c) {
    ...
    sdsfree(c->peerid);
    zfree(c);
}//end of void freeClient(client *c)
...
int processInlineBuffer(client *c) {
    ...
    zfree(argv);
    return C_OK;
}//end of int processInlineBuffer()
...
void readQueryFromClient(aeEventLoop *el, int fd, void *privdata, int mask) {
	...
	//printf("readQueryFromClient:c->flags:%d\n",c->flags);//0
	int nread, readlen;
	size_t qblen;
	UNUSED(el);
	UNUSED(mask);
...
int processEventsWhileBlocked(void) {
    ...
    int count = 0;
    // 具体来说，我们尝试将事件循环调用4次，只要我们收到确认某些事件被处理，
	// 以便继续接受，读取，写入，关闭序列来为客户端提供服务。
    while (iterations--) {
        int events = 0;
        // 处理发生的文件事件，且非阻塞
