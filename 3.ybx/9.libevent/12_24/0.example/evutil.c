...
err:
	...
	return -1;
}//end evutil_socket_connect


 //函数evutil_socket_finished_connecting会检查这个socket，从而得知这个socket是处于什么状态。
//在bufferevent_socket_connect函数中，出现的一些错误，比如被拒绝，也是能通过这个函数检查出来的。所以可以在这里做统一的处理。该函数的内部是使用
