...
void display_semarr(int semid){
	...
	printf("display semarr:");
	for(i=0; i<2; i++){
		printf("index:%d value:%d ",i ,arg1.array[i]);
	}
	printf("\n");
	free(arg1.array);
}
...
void P(int semid, int process_resource[][2], int process_num){
	...
	sf_arr[0].sem_flg = 0;
	//sf_arr[0].sem_flg = IPC_NOWAIT;
	sf_arr[1].sem_num = 1;
	sf_arr[1].sem_op = -process_resource[process_num][1];
	sf_arr[1].sem_flg = 0;//默认是阻塞
	//sf_arr[1].sem_flg = IPC_NOWAIT;//设置成非阻塞 参考hplsp page 246
	...
	while(semop(semid, sf_arr, 2)<0){
		if (errno == EINTR) {
			...
			//打印出在设置为非阻塞时候谁在获取不到足够的数据
			printf("pid:%d EAGAIN\n", getpid());
			sleep(1);//避免屏幕被刷满
			continue;
		}else if(errno == EIDRM){
			printf("pid :%d,semop() error is EIDRM\n", getpid());
			exit(-1);
		}else{
			perror("semop():");
			exit(-1);
...
static void stdio_func(int semid, int process_resource[][2], \
		int process_num){
	printf("pid:%d, ppid:%d block\n", getpid(), getppid());
	//printf("%d, block\n", process_num);
	P(semid,  process_resource, process_num);
	printf("pid:%d, ppid:%d unblock\n", getpid(), getppid());
	//printf("%d, unblock\n", process_num);
	sleep(5);
