...
void *thr_relayer(void *p)
{
	...
	//epoll的优点,只添加一次,while循环中根据具体情况去修改
	//布置监视任务ADD
	for(i = 0 ; i < REL_JOBMAX; i++)
	{
		if(rel_job[i] != NULL)
		{
			...
			{
				ev.data.fd = rel_job[i]->fd1;
				ev.events = 0;
				epoll_ctl(epfd, EPOLL_CTL_ADD, rel_job[i]->fd1, &ev);
				ev.data.fd = rel_job[i]->fd2;
				ev.events = 0;
				epoll_ctl(epfd, EPOLL_CTL_ADD, rel_job[i]->fd2, &ev);
			}
		}
	}
	...
	while(1)
	{
		...
		for(i = 0 ; i < REL_JOBMAX; i++)
		{
			if(rel_job[i] != NULL)
			{
				if(rel_job[i]->job_state == STATE_RUNNING)
				{
					...
					epoll_ctl(epfd, EPOLL_CTL_MOD, rel_job[i]->fd1, &ev);
					...
					epoll_ctl(epfd, EPOLL_CTL_MOD, rel_job[i]->fd2, &ev);
		...
		for(i = 0 ; i < REL_JOBMAX ; i++)
		{
			if(rel_job[i] != NULL)
			{
				if(rel_job[i]->job_state == STATE_RUNNING)
				{
					if ( ev.data.fd == rel_job[i]->fd1 && ev.events & EPOLLIN ||\
						 ev.data.fd == rel_job[i]->fd2 && ev.events & EPOLLOUT ) {
						fsm_driver(&rel_job[i]->fsm12);
					}
					if ( ev.data.fd == rel_job[i]->fd2 && ev.events & EPOLLIN ||\
						 ev.data.fd == rel_job[i]->fd1 && ev.events & EPOLLOUT ) {
						fsm_driver(&rel_job[i]->fsm21);
					}
				}
			}
